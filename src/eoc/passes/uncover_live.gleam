...
import eoc/langs/x86_var.{type Location, Block, LocReg, LocVar}
import gleam/dict
import gleam/list
import gleam/set
...
pub fn uncover_live(input: x86_var.X86Program) -> x86_var.X86Program {
  ...
  |> dict.map_values(fn(_, block) {
    Block(block.body, compute_live_after(block.body))
  })
  |> x86_var.X86Program()
}
...
fn compute_live_after(instrs: List(x86_var.Instr)) -> List(set.Set(Location)) {
  list.fold_right(
    instrs,
    [set.from_list([LocReg(x86_var.Rax), LocReg(x86_var.Rsp)])],
    fn(live_after, instr) {
...
fn locations_in_arg(arg: x86_var.Arg) -> set.Set(Location) {
  case arg {
...
fn read_locations_in_inst(inst: x86_var.Instr) -> set.Set(Location) {
  case inst {
    ...
    x86_var.Popq(_) -> set.from_list([LocReg(x86_var.Rsp)])
    x86_var.Pushq(a) ->
      set.union(locations_in_arg(a), set.from_list([LocReg(x86_var.Rsp)]))
    x86_var.Callq(_, arity) ->
      ...
      |> list.take(arity)
      |> set.from_list()
    x86_var.Retq -> set.from_list([LocReg(x86_var.Rax)])
    // Correct?
...
fn write_location_in_inst(inst: x86_var.Instr) -> set.Set(Location) {
  case inst {
    x86_var.Addq(_, b) -> locations_in_arg(b)
    x86_var.Subq(_, b) -> locations_in_arg(b)
    x86_var.Callq(_, _) -> set.from_list([LocReg(x86_var.Rax)])
