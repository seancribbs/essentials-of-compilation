...
import eoc/ui/colour.{type ColourPalette, type ColourScale, ColourPalette}
...
pub fn decoder(json: Dynamic) -> Result(Theme, List(DecodeError)) {
  decode.run(json, theme_decoder())
}

fn theme_decoder() -> decode.Decoder(Theme) {
  let tuple2 = fn(a, b) {
    use av <- decode.field(0, a)
    use bv <- decode.field(1, b)
    decode.success(#(av, bv))
  }
  ...
  use dark <- decode.field(
    "dark",
    decode.optional(tuple2(selector_decoder(), colour.palette_decoder())),
  )
  decode.success(Theme(id:, selector:, font:, radius:, space:, light:, dark:))
}

fn selector_decoder() -> decode.Decoder(Selector) {
  ...
  case kind {
    ...
    _ -> {
      decode.failure(Global, "'Global' | 'Class' | 'DataAttribute'")
    }
  }
}

fn fonts_decoder() -> decode.Decoder(Fonts) {
...
fn sizes_decoder() -> decode.Decoder(SizeScale) {
  ...
  use xl2 <- decode.field("xl-2", decode.float)
  use xl3 <- decode.field("xl-3", decode.float)
  decode.success(SizeScale(xs, sm, md, lg, xl, xl2, xl3))
}
...
pub const font = FontVariables(
  ...
  body: "var(--lustre-ui-font-body)",
  code: "var(--lustre-ui-font-code)",
)
