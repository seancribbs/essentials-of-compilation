...
pub fn format_program(input: X86Program) -> doc.Document {
  input.defs
  |> list.map(format_definition)
  |> doc.concat_join(with: [doc.line, doc.line])
}

pub fn format_definition(definition: Definition) -> doc.Document {
  definition.blocks
  |> dict.to_list
  |> list.map(format_block(_, definition.label))
  |> doc.concat_join(with: [doc.line, doc.line])
}
...
pub fn format_block(
  ...
  let prefix = case block_name == definition {
    True ->
      doc.concat([doc.from_string("\t.globl .align 8 " <> definition), doc.line])
    _ -> doc.empty
  }
  let label =
    doc.concat([
