import eoc/langs/l_fun.{type Cmp, type Type} as l
...
pub type Expr {
  ...
  Call(function: Atm, arguments: List(Atm))
}
...
pub type Tail {
  ...
  TailCall(function: Atm, arguments: List(Atm))
}

pub type Blocks {
  Blocks(blocks: dict.Dict(String, Tail), prefix: String)
}
...
pub type CProgram {
  CProgram(info: dict.Dict(String, List(String)), defs: List(Definition))
}

pub fn format_program(input: CProgram) -> doc.Document {
  doc.concat(list.map(input.defs, format_definition))
}

fn format_definition(def: Definition) -> doc.Document {
  let header =
    [
      doc.from_string(";;"),
      doc.from_string(def.name),
      doc.concat_join(list.map(def.arguments, l.format_argument), with: [
        doc.space,
      ]),
      doc.from_string(":"),
      l.format_type(def.return),
      doc.line,
    ]
...
fn format_tail(t: Tail) -> doc.Document {
  case t {
    ...
    TailCall(function:, arguments:) ->
      doc.concat([
        doc.from_string("tailcall "),
        format_atm(function),
        doc.from_string("("),
        doc.concat_join(list.map(arguments, format_atm), with: [
          doc.from_string(", "),
        ]),
        doc.from_string(");"),
      ])
  ...
  case e {
    ...
    FunRef(name:, arity:) ->
      doc.concat([
        ...
        doc.from_string("/"),
        doc.from_string(int.to_string(arity)),
      ])
    Call(function:, arguments:) ->
      doc.concat([
        doc.from_string("call "),
        format_atm(function),
        doc.from_string("("),
        doc.concat_join(list.map(arguments, format_atm), with: [
          doc.from_string(", "),
