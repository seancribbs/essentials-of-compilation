...
pub fn l_if_typecheck_program_test() {
  ...
  assert type_check_program(Program(Bool(True))) == Error(TypeError(Integer, Boolean, Bool(True)))
  //
  // (and #f #t)
  // => TypeError(Integer, Boolean, Prim(And(Bool(False), Bool(True))))
  assert type_check_program(Program(Prim(And(Bool(False), Bool(True))))) == Error(TypeError(Integer, Boolean, Prim(And(Bool(False), Bool(True)))))
  //
  // (if #t #f #f)
  // => TypeError(Integer, Boolean, If(Bool(True), Bool(False), Bool(False)))
  assert type_check_program(Program(If(Bool(True), Bool(False), Bool(False)))) == Error(TypeError(Integer, Boolean, If(Bool(True), Bool(False), Bool(False))))
  //
  // (let [x 42] (< 42 5))
  // => TypeError(Integer, Boolean, Let("x", Int(42), Prim(Cmp(Lt, Int(42), Int(5))))
  assert type_check_program(Program(Let("x", Int(42), Prim(Cmp(Lt, Int(42), Int(5)))))) == Error(TypeError(
      Integer,
      Boolean,
...
pub fn l_if_typecheck_expressions_test() {
  // (if (< 42 #t) 0 1)
  // => TypeError(Integer, Boolean, Bool(True))
  assert type_check_program(Program(If(Prim(Cmp(Lt, Int(42), Bool(True))), Int(0), Int(1)))) == Error(TypeError(Integer, Boolean, Bool(True)))

  // (let (a (+ 5 10)) (= a #f))
  // => TypeError(Integer, Boolean, Prim(Cmp(Eq, Var("a"), Bool(False))))
  assert type_check_program(Program(Let(
    "a",
  ...
  assert type_check_program(Program(If(Prim(Cmp(Eq, Int(10), Int(2))), Bool(False), Int(42)))) == Error(TypeError(Boolean, Integer, Int(42)))

  // (if (and #f 42) 1 2)
  // => TypeError(Boolean, Integer, Int(42))
  assert type_check_program(Program(If(Prim(And(Bool(False), Int(42))), Int(1), Int(2)))) == Error(TypeError(Boolean, Integer, Int(42)))

  // (if (or (read) #t) 0 1)
  // => TypeError(Boolean, Integer, Prim(Read))
  assert type_check_program(Program(If(Prim(Or(Prim(Read), Bool(True))), Int(0), Int(1)))) == Error(TypeError(Boolean, Integer, Prim(Read)))

  // UnboundVariable
  assert type_check_program(Program(Prim(Plus(Var("a"), Int(2))))) == Error(UnboundVariable("a"))
}

// Test: happy path cases
pub fn l_if_typecheck_happy_path_test() {
  // (let [x (read)] (if (>= 42 x) (+ x 5) (- x 10)))
  let assert Ok(_) = type_check_program(Program(Let(
    "x",
    Prim(Read),
    If(
