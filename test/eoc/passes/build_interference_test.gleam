  ...
  let main =
    x86.Definition(
      ...
      blocks: dict.from_list([
        #("main", Block(..base_block, body: instrs, live_after:)),
      ]),
    )

  let p = X86Program([main])

  let assert x86.X86Program([main2]) = build_interference.build_interference(p)
  let conflicts = main2.conflicts
...
pub fn build_interference_vector_with_calls_test() {
