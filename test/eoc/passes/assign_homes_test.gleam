...
pub fn assign_homes_test() {
  let base_block = var.new_block()
  let vp =
    var.X86Program(
      dict.from_list([
        #(
          "start",
          var.Block(..base_block, body: [
            ...
            var.Movq(var.Var("y.3"), var.Reg(Rax)),
            var.Jmp("conclusion"),
          ]),
        ),
      ]),
      ...
      dict.from_list([
        #(
          ...
          var.Block(..base_block, body: [
            ...
            var.Movq(var.Imm(42), var.Reg(Rax)),
            var.Addq(var.Var("tmp.1"), var.Reg(Rax)),
            var.Jmp("conclusion"),
          ]),
        ),
      ]),
    ...
    x86.X86Program(
      dict.from_list([
        #(
          ...
          x86.Block(
            [
              x86.Movq(x86.Imm(10), x86.Deref(Rbp, -8)),
              x86.Negq(x86.Deref(Rbp, -8)),
              x86.Movq(x86.Imm(42), x86.Reg(Rax)),
              x86.Addq(x86.Deref(Rbp, -8), x86.Reg(Rax)),
              x86.Jmp("conclusion"),
            ],
            8,
          ),
